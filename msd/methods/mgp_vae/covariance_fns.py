...
def covariance_function(p_type, nlen, H=None, fac=0.1):
    if p_type == 'frac':
        if H is None:
            raise Exception("Invalid Argument H")
        return gen_frac_cov(H, nlen, fac=fac)

    elif p_type == 'bb':
        return gen_BB_cov(nlen, fac=fac)

    elif p_type == 'bb2':
        return gen_BB2_cov(nlen, fac=fac)

    elif p_type == 'ou':
        return gen_OU_cov(nlen, fac=fac)

    elif p_type == 'slow':
        return gen_slowfeature(nlen, fac=fac)
